#!/bin/bash
# Warm SFT v6 L=2 with --num_epochs 50 — paired with launch_warm_sft_v6_L4_ep50.
# Same recipe (num_target_features=5, warmup_ratio=0.04, random_mask off,
# allow_partial, overlapping_blocks) but draft depth = 2.
...
RUN=dflash_L2_lr1e-4_ep50_bs16_warm_v6_sharon1
OUTDIR=$DATA/runs/$RUN
mkdir -p $OUTDIR

cd /home/ubuntu/katana_transfer/code
torchrun --nproc_per_node=4 --master_port=29637 \
