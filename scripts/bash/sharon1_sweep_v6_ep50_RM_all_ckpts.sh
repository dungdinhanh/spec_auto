#!/bin/bash
# Sweep every epoch ckpt of v6 ep50 + random_mask (L=2 and L=4) on val/test/off-shelf.
# 50 ckpts × 2 runs × 3 splits = 300 evals, distributed round-robin across 8 GPUs.
set -e
...
L4_RUN=$LOCAL/runs/dflash_L4_lr1e-4_ep50_bs16_warm_v6_randomMask_sharon1
L2_RUN=$LOCAL/runs/dflash_L2_lr1e-4_ep50_bs16_warm_v6_randomMask_sharon1
...
run_gpu() {
  ...
  while IFS='|' read -r ck nlayers cdir uuids outpref; do
    ...
    echo "[gpu$gpu] RUN $(basename $outpref) layers=$nlayers"
...
for g in $(seq 0 $((N_GPUS-1))); do
  awk -v g=$g -v n=$N_GPUS 'NR % n == g' $JOBS_FILE | run_gpu $g > /tmp/sweep_RM_gpu${g}.out 2>&1 &
  echo "Launched GPU $g (jobs: $(awk -v g=$g -v n=$N_GPUS 'NR % n == g' $JOBS_FILE | wc -l))"
done
wait
rm -f $JOBS_FILE
echo "SWEEP_RM_DONE $(date)"
