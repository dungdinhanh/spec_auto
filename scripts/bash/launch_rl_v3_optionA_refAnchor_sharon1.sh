#!/bin/bash
# v3 Option A on sharon1, but with --anchor_source ref (static-ref KL anchor).
# Runs in PARALLEL with the policy-anchor Option A on GPUs 0-3 — this one uses 4-7.
# Reward = w_traj·(-MSE) + w_text·token_overlap (Option A: w_cons=0).
...
torchrun --nproc_per_node=4 --master_port=29613 \
    ...
    --output_dir /home/ubuntu/local_data/runs/rl_v3_optionA_K5_refAnchor \
...
echo "V3_OPTIONA_REFANCHOR_DONE $(date)"
