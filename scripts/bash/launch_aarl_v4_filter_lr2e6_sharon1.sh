#!/bin/bash
# v4 AARL with --filter_to_rejection_blocks on, GPUs 4-7, lr=2e-6 (2x v3
# baseline). Hypothesis: with denser gradient (no zero-rl steps), the policy
# can support a larger step size without instability.
...
torchrun --nproc_per_node=4 --master_port=29612 \
    ...
    --output_dir /home/ubuntu/local_data/runs/aarl_v4_filter_lr2e-6 \
    --num_epochs 2 --lr 2e-6 --kl_weight 0.02 \
...
echo "V4_FILTER_LR2E6_DONE $(date)"
