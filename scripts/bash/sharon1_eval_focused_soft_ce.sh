...
D=$LOCAL/runs/dflash_L4_focused_soft_ce_warm_init

CK_GPU0=( $D/draft_final.pt $D/draft_topk_step_400_wce1.8161.pt $D/draft_topk_step_200_wce1.9444.pt )
CK_GPU1=( $D/draft_topk_step_800_wce2.2840.pt $D/draft_epoch_1.pt )
...
run_chunk() {
  ...
  CUDA_VISIBLE_DEVICES=$gpu nohup $PYBIN $SCRIPTS/eval_ckpt_sweep_vt.py \
    ...
    --target_outputs_dir $COC --val_uuids_file $VAL --test_uuids_file $TEST \
    --output_csv $OUTDIR/eval_gpu${gpu}.csv > $OUTDIR/eval_gpu${gpu}.log 2>&1 &
