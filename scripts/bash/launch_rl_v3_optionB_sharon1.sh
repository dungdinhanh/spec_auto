#!/bin/bash
# v3 Option B on sharon1: ref-anchor + meta-action consistency reward.
# Hyperparameters matched to winning Option A config (lr=1e-6, N=3, K=5, 2 epochs).
# Reward = w_traj·(-MSE) + w_cons·1[meta match] + w_text·token_overlap.
...
torchrun --nproc_per_node=4 --master_port=29614 \
    ...
    --output_dir /home/ubuntu/local_data/runs/rl_v3_optB_ref_lr1e-6_N3 \
    --num_epochs 2 --lr 1e-6 --kl_weight 0.02 \
    --k_samples 5 --k_chunk_size 5 --temperature 1.0 \
    --w_traj 1.0 --w_cons 0.1 --w_text 0.5 \
    --enable_r_cons --consistency_horizon 16 --eps_long 0.05 --eps_lat 0.10 \
    --contamination_N 3 \
    --anchor_source ref \
    --log_interval 5 --save_interval 500 --no_wandb
echo "V3_OPTIONB_DONE $(date)"
