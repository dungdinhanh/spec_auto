#!/bin/bash
# SOFT focused-CE: at rejected positions use soft CE against target's full
# distribution (not just GT one-hot). KL anchor at matched positions vs warm SFT.
# Same hyperparams as hard focused-CE: lr=1e-5, kl_weight=1.0, 3 epochs.
# Runs on sharon1 GPUs 0-3 (K=30 RL is on GPUs 4-7).
set -e
...
OUT=$LOCAL/runs/dflash_L4_focused_soft_ce_warm_init
LOG=/tmp/dflash_L4_focused_soft_ce.log
mkdir -p $OUT
...
CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 --master_port=29645 \
...
sleep 1
ps -ef | grep "use_soft_target_ce" | grep -v grep | head -3
