#!/bin/bash
# Eval warm SFT v6 draft_final on val/test/off-shelf using e2e_spec_test.py.
# 1D FlatRoPE (no --use_mrope*_draft flag). 3 splits in parallel on GPUs 0-2.
set -e
...
RUN=$LOCAL/runs/dflash_L4_lr1e-4_ep15_bs16_warm_v6_sharon1
OUT=$RUN/_eval_1d
mkdir -p $OUT
...
echo "EVAL_V6_LAUNCHED pids=$P0,$P1,$P2"
wait
echo "EVAL_V6_DONE $(date)"
echo "--- summary ---"
for f in $OUT/*.log; do
  tag=$(basename $f .log)
