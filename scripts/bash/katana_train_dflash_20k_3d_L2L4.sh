...
#PBS -j oe
#PBS -N dflash20k_3d_L2L4
# Full-22k DFlash training with 3D M-RoPE on the H200 reservation node (k201).
# ONE job, 8 GPUs: L=2 on GPUs 0-3, L=4 on GPUs 4-7, in parallel.
# CE-only (new 10k clips have no cached logits); v6-RM recipe + --use_mrope3d_draft.
set -e
...
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29661 \
    ...
    --num_draft_layers 2 \
    --output_dir $SCRATCH/runs/dflash_L2_20k_ep30_ceonly_3d_katana \
    $(common_args) > $SCRATCH/runs/dflash_20k_3d_L2.log 2>&1 &
P2=$!

CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=29662 \
    scripts/train_dflash_distillation_v7.py \
    --num_draft_layers 4 \
    --output_dir $SCRATCH/runs/dflash_L4_20k_ep30_ceonly_3d_katana \
    $(common_args) > $SCRATCH/runs/dflash_20k_3d_L4.log 2>&1 &
P4=$!

echo "Launched L2(pid=$P2) and L4(pid=$P4) with 3D M-RoPE at $(date)"
wait $P2; echo "L2 exit=$?"
wait $P4; echo "L4 exit=$?"
echo "DONE_DFLASH_20k_3d_L2L4 $(date)"
