#!/bin/bash
# v7 L=4 variant A — 70% full mask + 30% discrete-mid {4,8,11}, RANDOM positions.
# Runs on sharon1 GPUs 0-3 (4 GPUs).
set -e
...
RUN=dflash_L4_lr1e-4_ep50_bs16_warm_v7_varA_sharon1
OUTDIR=$DATA/runs/$RUN
mkdir -p $OUTDIR

cd /home/ubuntu/katana_transfer/code
torchrun --nproc_per_node=4 --master_port=29640 \
    ...
    --full_mask_prob 0.7 --discrete_levels 4 8 11 \
    --kl_weight 1.0 \
    --warm_start --seed 42 --no_wandb
echo "DONE_V7_L4_varA $RUN $(date)"
