#!/bin/bash
# Eval warm SFT v6 L=2 ep50 + random_mask draft_final on val/test/off-shelf.
# 1D FlatRoPE, num_target_features=5. 3 splits in parallel on GPUs 4-6.
set -e
...
RUN=$LOCAL/runs/dflash_L2_lr1e-4_ep50_bs16_warm_v6_randomMask_sharon1
OUT=$RUN/_eval_1d
mkdir -p $OUT
...
CUDA_VISIBLE_DEVICES=4 nohup $PYBIN $SCRIPTS/e2e_spec_test.py \
...
CUDA_VISIBLE_DEVICES=5 nohup $PYBIN $SCRIPTS/e2e_spec_test.py \
  --target_path $TARGET --draft_path $DRAFT \
...
CUDA_VISIBLE_DEVICES=6 nohup $PYBIN $SCRIPTS/e2e_spec_test.py \
  --target_path $TARGET --draft_path $DRAFT \
...
echo "EVAL_V6_L2_EP50_RANDOMMASK_LAUNCHED pids=$P0,$P1,$P2"
wait
echo "EVAL_V6_L2_EP50_RANDOMMASK_DONE $(date)"
echo "--- summary ---"
for f in $OUT/*.log; do
  tag=$(basename $f .log)
