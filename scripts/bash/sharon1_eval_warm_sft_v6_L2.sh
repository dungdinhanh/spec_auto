#!/bin/bash
# Eval warm SFT v6 L=2 draft_final on val/test/off-shelf using e2e_spec_test.py.
# 1D FlatRoPE. 3 splits in parallel on GPUs 4-6.
set -e
...
RUN=$LOCAL/runs/dflash_L2_lr1e-4_ep15_bs16_warm_v6_sharon1
OUT=$RUN/_eval_1d
mkdir -p $OUT
...
echo "EVAL_V6_L2_LAUNCHED pids=$P0,$P1,$P2"
wait
echo "EVAL_V6_L2_DONE $(date)"
echo "--- summary ---"
for f in $OUT/*.log; do
  tag=$(basename $f .log)
