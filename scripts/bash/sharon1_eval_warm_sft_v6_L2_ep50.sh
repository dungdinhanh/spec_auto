#!/bin/bash
# Eval warm SFT v6 L=2 ep50 draft_final on val/test/off-shelf via e2e_spec_test.py.
...
RUN=$LOCAL/runs/dflash_L2_lr1e-4_ep50_bs16_warm_v6_sharon1
OUT=$RUN/_eval_1d
mkdir -p $OUT
...
echo "EVAL_V6_L2_EP50_LAUNCHED pids=$P0,$P1,$P2"
wait
echo "EVAL_V6_L2_EP50_DONE $(date)"
echo "--- summary ---"
for f in $OUT/*.log; do
  tag=$(basename $f .log)
