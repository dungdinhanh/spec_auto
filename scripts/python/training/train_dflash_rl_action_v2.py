"""RL fine-tuning of a DFlash draft with trajectory-action-MSE as reward — v2.

Differences from v1 (train_dflash_rl_action.py):
  * Stochastic sampling is restricted to the positions where the draft's GREEDY
    prediction disagrees with the target greedy CoC. Greedy-matched positions
    use the deterministic greedy token and contribute NO log_prob to the policy
    gradient — there's no useful learning signal at positions the draft
    already gets right, and including them just inflates gradient variance.
  * KL(π||π_ref) is computed only over greedy-matched positions. Rationale: at
    those positions we want to keep the draft's distribution close to its
    initial distillation (anchor); at rejected positions the draft is wrong
    anyway and we want it free to learn. KL on rejected positions would
    actively penalize the very moves the policy gradient is trying to make.

Design (carried over from v1):
  1. Reward  = − MSE(action_gt, action_mix) in normalized (accel, curvature) space,
              optionally + accept_bonus * accepted_length.
  2. Block   = block_start ∼ P(b) ∝ decay^b (early positions sampled more often).
  3. KL ref  = frozen copy of init, kl_weight default 0.02.
  4. Sample T= 1.0 during RL; argmax at inference.
  5. K       = 4 samples per step, batched through target VLM + diffusion.

Per training step:
  Pass A (no grad):
     VLM forward(prompt + gt_coc + <|traj_future_start|>) → cache A
  ...
  Sampling:
     ...
     rejected_mask = (greedy[:gt_available] != target_greedy_block)
     at rejected positions: K stochastic samples; at matched positions: greedy×K
     seq_log_probs = sum_{p ∈ rejected} log π(sampled[p,k] | …)   ← only rejected
  Build K mixed-CoC by substituting first contamination_N rejected positions
  with each K's stochastic sample.
  Pass B (no grad): K-batched VLM + diffusion → action_mix_k → reward_k.
  Loss:
     advantage = reward − group_mean(reward)
     rl_loss   = − E[advantage · seq_log_probs]
     kl_loss   = mean over MATCHED positions of KL(π_current || π_ref)
...
def rl_step(
    ...
    # KL(pi_current || pi_ref) averaged over GREEDY-MATCHED positions only.
    # At rejected positions the policy is intentionally drifting away from the
    # init/ref distribution to find better tokens — penalizing that with KL would
    # cancel the very updates the policy gradient is trying to make. KL acts as
    # an anchor only on positions the draft already gets right, keeping its
    # behavior at those positions stable.
    with torch.no_grad():
    ...
    n_matched = int(matched_full.sum().item())
    if n_matched > 0:
        kl_loss = (kl_per_pos * matched_full.float()).sum() / n_matched
    else:
...
def main():
    ...
    ap.add_argument("--k_chunk_size", type=int, default=4,
                    help="Chunk size along the K dimension when running target VLM + diffusion. "
                         "Memory peaks at chunk_K samples through the target. "
                         "Default 4 matches original K=4 memory profile.")
