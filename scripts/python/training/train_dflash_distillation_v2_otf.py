"""DFlash block diffusion draft training for Alpamayo-R1 — soft-CE OTF variant.

Fork of `train_dflash_distillation_v2.py`. Identical block-diffusion machinery,
identical dataset (cached `target_coc_outputs/`), identical hard-CE loss.

Adds one option: `--soft_ce_otf` for the soft-CE / KL distillation term.

  - Default (cached, original v2): KL between draft logits at output positions
    and `output_logits` cached in `target_coc_outputs/*.pt`.
  - `--soft_ce_otf`: KL against TARGET LOGITS computed fresh on-the-fly from
    the same target forward pass we already do for hidden states. No new
    forward; just slice `target_out.logits[:, prompt_len:, :]`. Mimics
    EAGLE-3's training-time soft KL — uses the live target distribution
    rather than a cached snapshot.

The two losses (hard CE on cached output_token_ids + soft CE on fresh target
logits) compose with `--kl_weight`, exactly as in v2. Logging now reports
both `ce` (hard) and `kl` (soft) separately.
...
def train_step(
    ...
    for b in range(B):
        ...
        for start in starts_b:
            ...
            if use_mrope3d_draft:
                # 3D M-RoPE: slice precomputed (3, B, L_max) for sample b, this window.
                pos_ids = full_3d_pos[:, b:b + 1, :ctx_len + block_size]
            else:
    ...
    # KL / soft-CE distillation loss against target logits.
    #
    # Two sources for the target distribution:
    #   - cached (default): target_logits_list from collate (`output_logits`
    #     stored in target_coc_outputs/*.pt at generation time).
    #   - on-the-fly (`soft_ce_otf=True`, EAGLE-3-style): slice
    #     `target_out.logits[b, p_len:, :]` from the SAME target forward we
    #     already ran for hidden states. No new forward; just uses the live
    #     target distribution that's already computed at all output positions.
    #
    # Both paths compute KL = E_target_p [ log target_p − log draft_p ] at
    # the predicted (next-token) positions of each block, averaged across
    # eligible (block, position) pairs.
    kl_weight = batch.get("_kl_weight", 0.0)
    soft_ce_otf = batch.get("_soft_ce_otf", False)
    if kl_weight > 0:
        cached_logits_list = batch.get("output_logits_list")
        fresh_logits_list = None
        if soft_ce_otf:
            # Fresh target logits from the forward we already did. Build a
            # per-sample list to share the existing per-block KL code below.
            fresh_logits_list = []
            for b in range(B):
                p_len_b = int(prompt_lens[b])
                seq_len_b = int(seq_lens[b])
                if p_len_b <= 0 or seq_len_b <= p_len_b:
                    fresh_logits_list.append(None)
                    continue
                # target_out.logits[b, k] is the prediction for position k+1 in
                # input_ids (i.e., the next-token distribution given inputs
                # 0..k). The cached `output_logits` for output_pos j is the
                # prediction for input_ids[p_len+j] given 0..p_len+j-1, which
                # equals target_out.logits[b, p_len+j-1]. So slice at
                # [p_len-1..seq_len-2].
                t_slice = target_out.logits[b,
                                            p_len_b - 1:seq_len_b - 1, :].detach()
                fresh_logits_list.append(t_slice)
            target_logits_list = fresh_logits_list

            # Comparison: when both sources are available, log per-step diff
            # metrics so we can see if cached and fresh align numerically.
            if cached_logits_list is not None:
                with torch.no_grad():
                    diff_max_abs = 0.0
                    diff_mean_abs = 0.0
                    arg_agree_num = 0
                    arg_agree_den = 0
                    for b in range(B):
                        c = cached_logits_list[b] if cached_logits_list else None
                        fr = fresh_logits_list[b]
                        if c is None or fr is None:
                            continue
                        c = c.to(device).float()
                        fr_f = fr.float()
                        # The cached side is shaped (num_generated, V); the
                        # fresh side is (seq_len_b - p_len_b, V) which can
                        # differ by 1 if padding makes seq_len_b > p_len_b +
                        # num_generated. Truncate to the min len of both.
                        n_cmp = min(c.shape[0], fr_f.shape[0])
                        if n_cmp <= 0:
                            continue
                        c2 = c[:n_cmp]
                        f2 = fr_f[:n_cmp]
                        d = (c2 - f2).abs()
                        diff_max_abs = max(diff_max_abs, d.max().item())
                        diff_mean_abs += d.mean().item()
                        arg_agree_num += (c2.argmax(-1) == f2.argmax(-1)).sum().item()
                        arg_agree_den += n_cmp
                    losses["debug_otf_max_abs"] = torch.tensor(diff_max_abs)
                    losses["debug_otf_mean_abs"] = torch.tensor(
                        diff_mean_abs / max(B, 1))
                    losses["debug_otf_arg_agree"] = torch.tensor(
                        arg_agree_num / max(arg_agree_den, 1))
        else:
            target_logits_list = cached_logits_list

        if target_logits_list is not None:
            ...
            for bi, (b_idx, start) in enumerate(all_block_starts):
                ...
                for k in range(block_size - 1):
                    ...
                    if 0 <= tgt_idx < t_logits.shape[0]:
                        draft_lp = F.log_softmax(
                            all_draft_logits[bi][0, k, :].float(), dim=-1)
...
def main():
    ...
    parser.add_argument("--soft_ce_otf", action="store_true",
                        help="Use FRESH target logits (sliced from the same target forward "
                             "we already do for hidden states) for the KL term, instead of "
                             "the cached `output_logits` from target_coc_outputs/*.pt. "
                             "Mimics EAGLE-3 training-time soft KL: live target distribution "
                             "rather than a frozen snapshot. Cached output_token_ids are still "
                             "used for the hard CE loss.")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, batch in enumerate(loader):
            ...
            try:
                ...
                batch["_soft_ce_otf"] = args.soft_ce_otf
                ...
                epoch_uce += step_uce
                epoch_kl += step_kl
                # Debug: cached vs OTF logits comparison metrics (only set
                # when both are available, i.e., --soft_ce_otf with cached
                # output_logits in the dataset).
                dbg_max = losses.get("debug_otf_max_abs")
                dbg_mean = losses.get("debug_otf_mean_abs")
                dbg_arg = losses.get("debug_otf_arg_agree")
                if dbg_max is not None and is_main_process(rank):
                    print(f"  [otf-vs-cached] step {global_step+1}: "
                          f"max|Δ|={dbg_max.item():.4f}  "
                          f"mean|Δ|={dbg_mean.item():.5f}  "
                          f"argmax_agree={dbg_arg.item():.4f}", flush=True)
                epoch_count += 1
