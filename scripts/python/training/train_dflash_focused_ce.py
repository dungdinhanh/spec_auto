"""DFlash focused-CE fine-tuning of warm SFT.

Forked from train_dflash_distillation_v3.py. Loss design:
  * At positions where draft greedy = GT (matched): apply KL(draft || ref)
    where ref = frozen warm SFT init. Anchor — keeps draft's behavior stable
    on the ~68% of positions it already gets right.
  * At positions where draft greedy != GT (rejected): apply hard CE
    (-log p_draft(GT_token)). Push draft toward correct answer.
  * Total loss = ce_on_rejected.mean() + kl_weight * kl_on_matched.mean()

Both masks are dynamic per step — recomputed using the current draft's argmax.
As the draft improves, the rejected set shrinks and the gradient naturally
focuses on remaining hard cases.

Same data pipeline (target_outputs_dir), DDP, topk_save_val infra, and most
hyperparams as the v3 SFT script. The differences vs v3:
  * Always loads a frozen reference draft (--ref_draft_path)
  * Replaces target-logit KL with reference-draft KL on matched positions only
  * Replaces full-position CE with rejected-position-only CE
...
def block_diffusion_loss(
    ...
    kl_weight: float = 0.0,
    target_logits: torch.Tensor = None,    # if provided, use soft CE against target's distribution at rejected positions
) -> dict[str, torch.Tensor]:
    """Focused-CE + KL anchor loss for masked block diffusion.

    Loss decomposition:
      - matched_mask[p]:  draft.argmax(p) == labels[p]   (valid positions only)
      - rejected_mask[p]: NOT matched_mask[p]            (valid positions only)
      - CE branch: hard CE applied ONLY at rejected positions, position-weighted
        (DFlash early-position bias).
      - KL branch: KL(draft || ref) ONLY at matched positions (anchor — keeps
        draft's distribution stable where it's already correct).

    The masks are dynamic per step using the current draft argmax — as the draft
    improves, the rejected set shrinks naturally.

    Args:
        draft_logits:  (B, L, V) — draft predictions
        labels:        (B, L)    — ground truth tokens (-100 = invalid/ignored)
        block_size:    DFlash block size (for position weights)
        ignore_index:  token ID to ignore (typically -100)
        ref_logits:    (B, L, V) — reference draft logits (frozen warm SFT). Required if kl_weight > 0.
        kl_weight:     weight for the KL-anchor branch
    """
    B, L, V = draft_logits.shape

    # Position weights w_k = exp(-(k)/block_size) tiled across L
    ...
    # ---- masks (dynamic per step) ----
    valid_mask_bool = (labels != ignore_index)            # (B, L)
    draft_argmax = draft_logits.argmax(dim=-1)            # (B, L)
    matched_mask_bool = valid_mask_bool & (draft_argmax == labels)
    rejected_mask_bool = valid_mask_bool & ~matched_mask_bool

    # ---- CE on REJECTED positions only ----
    rej_mask_f = rejected_mask_bool.float()
    rej_count = rej_mask_f.sum().clamp(min=1)
    if target_logits is not None:
        # SOFT CE: -sum_v p_target(v) * log p_draft(v) at each position
        draft_lp = F.log_softmax(draft_logits.float(), dim=-1)             # (B, L, V)
        target_p = F.softmax(target_logits.float(), dim=-1)                # (B, L, V) — no grad
        per_token_ce = -(target_p * draft_lp).sum(dim=-1)                   # (B, L)
        # Hard CE for logging comparison
        per_token_hard_ce = F.cross_entropy(
        ...
        per_token_hard_ce = per_token_ce
    weighted_ce = per_token_ce * position_weights.unsqueeze(0) * rej_mask_f
    ce_loss = weighted_ce.sum() / rej_count

    # ---- KL anchor on MATCHED positions only (vs ref) ----
    if kl_weight > 0 and ref_logits is not None:
        # Use full-precision softmax for numerical stability
        draft_lp = F.log_softmax(draft_logits.float(), dim=-1)        # (B, L, V)
        ref_p = F.softmax(ref_logits.float(), dim=-1)                  # (B, L, V) — no grad
        # KL(draft || ref) per position = sum_v ref(v) * (log ref(v) - log draft(v))
        # PyTorch convention: F.kl_div(log_target, input) actually computes target*(log target - input)
        # For KL(p || q) where draft=p, ref=q: = sum_v p(v) (log p(v) - log q(v)).
        # We want KL(draft || ref) = sum_v draft(v) * (log draft(v) - log ref(v))
        ref_lp = F.log_softmax(ref_logits.float(), dim=-1)
        draft_p = draft_lp.exp()
        kl_per_pos = (draft_p * (draft_lp - ref_lp)).sum(dim=-1)        # (B, L)
        match_mask_f = matched_mask_bool.float()
        match_count = match_mask_f.sum().clamp(min=1)
        kl_loss = (kl_per_pos * match_mask_f).sum() / match_count
    else:
        kl_loss = torch.tensor(0.0, device=draft_logits.device)

    # Total loss
    loss = ce_loss + kl_weight * kl_loss

    # Logging metrics (unweighted across valid positions for comparability)
    valid_count = valid_mask_bool.float().sum().clamp(min=1)
    unweighted_ce_all = (per_token_hard_ce * valid_mask_bool.float()).sum() / valid_count
    n_matched = matched_mask_bool.float().sum()
    n_rejected = rejected_mask_bool.float().sum()
    n_valid = valid_mask_bool.float().sum()

    return {
        "loss": loss,
        "weighted_ce": loss,            # alias for logging compat
        "unweighted_ce": unweighted_ce_all,
        "ce_loss": ce_loss.detach(),
        "kl_loss": kl_loss.detach() if torch.is_tensor(kl_loss) else torch.tensor(0.0, device=draft_logits.device),
        "n_matched": n_matched.detach(),
        "n_rejected": n_rejected.detach(),
...
def train_step(
    ...
    draft_ref: nn.Module = None,            # frozen warm SFT for KL anchor
    kl_weight: float = 0.0,
    use_soft_target_ce: bool = False,       # use soft CE against target logits at rejected positions
) -> dict:
    ...
    all_target_logits: list[torch.Tensor] = []   # for soft CE (aligned per-block)
    target_logits_list = batch.get("output_logits_list") if use_soft_target_ce else None
    ...
    for b in range(B):
        ...
        for start in starts_b:
            ...
            # Reference draft forward (no grad) for KL anchor on matched positions.
            if draft_ref is not None and kl_weight > 0:
                with torch.no_grad():
                    ref_hidden = draft_ref(
                        target_hidden=ctx_hidden,
                        noise_embedding=noise_embedding,
                        position_ids=pos_ids,
                    )
                    ref_block_logits = lm_head(ref_hidden[:, -(block_size - 1):, :])
            else:
                ref_block_logits = None
            ...
            if ref_block_logits is not None:
                if "all_ref_logits" not in locals():
                    all_ref_logits = []
                all_ref_logits.append(ref_block_logits)

            # Align target logits for this block (for soft-target CE)
            if use_soft_target_ce and target_logits_list is not None:
                t_logits = target_logits_list[b]
                V = block_logits.shape[-1]
                block_target = torch.zeros((1, block_size - 1, V), device=device, dtype=block_logits.dtype)
                if t_logits is not None:
                    t_logits_dev = t_logits.to(device)
                    for k in range(block_size - 1):
                        global_pos = start + k + 1
                        tgt_idx = global_pos - p_len
                        if 0 <= tgt_idx < t_logits_dev.shape[0]:
                            block_target[0, k, :] = t_logits_dev[tgt_idx, :V].to(block_target.dtype)
                all_target_logits.append(block_target)
    ...
    ref_logits_cat = torch.cat(all_ref_logits, dim=1) if 'all_ref_logits' in locals() and all_ref_logits else None
    target_logits_cat = torch.cat(all_target_logits, dim=1) if all_target_logits else None

    losses = block_diffusion_loss(
        draft_logits=draft_logits_cat,
        labels=labels_cat,
        block_size=block_size,
        ref_logits=ref_logits_cat,
        kl_weight=kl_weight,
        target_logits=target_logits_cat,
    )
...
def _run_validation(draft, draft_module, vlm, embed_tokens, lm_head,
        ...
        for i, sample in enumerate(val_loader):
            ...
            try:
                losses = train_step(
                    ...
                    use_soft_target_ce=args.use_soft_target_ce,
                )
                if losses is None:
                    continue
    ...
    if is_main_process(rank) and val_count > 0:
        ...
        return (v_wce, v_uce, val_count)
    return (None, None, 0)


def _maybe_topk_save(val_topk, val_wce, global_step, draft_module, args, output_dir, use_tmp):
    """Maintain a top-K (by lowest val_wce) list of saved checkpoints.
    val_topk: list of (val_wce, step, path) tuples; sorted ascending by val_wce.
    K = args.topk_save_val. If 0, no-op.
    """
    if args.topk_save_val <= 0 or val_wce is None:
        return val_topk
    K = args.topk_save_val
    path = os.path.join(output_dir, f"draft_topk_step_{global_step}_wce{val_wce:.4f}.pt")
    _safe_save(_wrap_draft_ckpt(draft_module.state_dict(), args), path, use_tmp=use_tmp)
    val_topk.append((val_wce, global_step, path))
    val_topk.sort(key=lambda x: x[0])
    while len(val_topk) > K:
        _, evict_step, evict_path = val_topk.pop()
        try:
            os.remove(evict_path)
            print(f"  topk: evicted {os.path.basename(evict_path)} (step {evict_step})")
        except OSError as e:
            print(f"  topk: failed to evict {evict_path}: {e}")
    print(f"  topk: kept {len(val_topk)}/{K} - " +
          " ".join(f"step{s}:wce{v:.4f}" for v, s, _ in val_topk))
    return val_topk
...
def main():
    ...
    parser.add_argument("--topk_save_val", type=int, default=0,
                        help="If >0, also keep the top-K checkpoints with lowest "
                             "val/weighted_ce (across all val firings: mid-epoch, end-of-epoch, "
                             "and final). Saved as draft_topk_step_<N>_wce<L>.pt; worst is evicted.")
    ...
    parser.add_argument("--use_soft_target_ce", action="store_true",
                        help="At rejected positions, use soft CE against target's full distribution "
                             "(loaded from output_logits_list) instead of hard CE against GT token. "
                             "Same KL anchor at matched positions.")

    parser.add_argument("--seed", type=int, default=42)
    # ---- EMA / periodic ref-update args (Option B defaults) ----
    parser.add_argument("--ref_update_interval", type=int, default=0,
                        help="Update reference draft every N training steps. 0 = static ref (default). "
                             "Recommended: 1000 for periodic updates.")
    parser.add_argument("--ref_update_mode", choices=["replace", "ema"], default="replace",
                        help="How to update ref. 'replace' copies trainable→ref; 'ema' blends.")
    parser.add_argument("--ref_ema_alpha", type=float, default=0.9,
                        help="EMA decay if mode=ema. Higher = slower update.")
    parser.add_argument("--ref_update_gate", choices=["none", "train_rolling_rate", "eval_acceptance"],
                        default="none",
    ...
    if is_main_process(rank):
        print(f"Building DFlash draft model ({'M-RoPE' if args.use_mrope_draft else '1D RoPE'})...")
    if args.use_mrope_draft:
    ...
    if args.warm_start:
        if is_main_process(rank):
            print(f"[warm_start] copying target.vlm.language_model.layers -> "
                  f"draft.layers via target_layer_ids={draft.target_layer_ids}")
        from alpamayo_r1.models.dflash_draft import warm_start_draft_from_target
        tgt_layers = vlm.language_model.layers
        warm_start_draft_from_target(draft, tgt_layers, draft.target_layer_ids,
    ...
    # --- Frozen reference draft (for KL anchor on matched positions) ---
    draft_ref = None
    if args.kl_weight > 0:
        if is_main_process(rank):
            print(f"[focused_ce] Loading frozen reference draft from {args.pretrained_draft or '(same warm_start init)'}")
        if args.use_mrope_draft:
            draft_ref = build_dflash_draft_mrope_for_qwen3vl(
                vlm, num_draft_layers=args.num_draft_layers,
                block_size=args.block_size, mask_token_id=args.mask_token_id,
                target_layer_ids=draft_module.target_layer_ids,
            )
        else:
            draft_ref = build_dflash_draft_for_qwen3vl(
                vlm, num_draft_layers=args.num_draft_layers,
                block_size=args.block_size, mask_token_id=args.mask_token_id,
                target_layer_ids=draft_module.target_layer_ids,
            )
        # Load same weights as init (warm SFT) — frozen
        if args.pretrained_draft:
            from alpamayo_r1.models.dflash_draft import load_draft_checkpoint
            ckpt = load_draft_checkpoint(args.pretrained_draft, map_location=device)
            draft_ref.load_state_dict(ckpt["state_dict"], strict=False)
        else:
            # No pretrained_draft → ref = current draft init state (warm-started)
            draft_ref.load_state_dict(draft_module.state_dict(), strict=False)
        for p in draft_ref.parameters():
            p.requires_grad_(False)
        draft_ref = draft_ref.to(device).eval()
        if is_main_process(rank):
            print(f"[focused_ce] Reference draft frozen on device.")
    ...
    val_topk = []        # top-K (lowest val/wce) checkpoints; list of (val_wce, step, path)
    if args.resume:
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, batch in enumerate(loader):
            ...
            try:
                ...
                # In focused-CE, the total loss is already CE + kl_weight*KL inside losses["loss"].
                # No additional kl term to add here.
            ...
            if (i + 1) % args.grad_accum_steps == 0:
                ...
                if (draft_ref is not None
                    ...
                    if args.ref_update_gate == "train_rolling_rate":
                        # focused-CE doesn't track an acc_rate metric; treat as no-op gate
                        should_update = True
                    if should_update:
                        ...
                        if is_main_process(rank):
                            print(f"  [ref_update] step={global_step} mode={args.ref_update_mode} "
                                  f"alpha={args.ref_ema_alpha}: ref updated.", flush=True)
            ...
            if (val_loader is not None and args.val_interval > 0
                ...
                v_wce, _, _ = _run_validation(draft, draft_module, vlm, embed_tokens, lm_head,
                                val_loader, args, device, epoch, global_step,
                ...
                if is_main_process(rank):
                    val_topk = _maybe_topk_save(val_topk, v_wce, global_step,
                                                draft_module, args, args.output_dir,
                                                args.save_via_tmp)
